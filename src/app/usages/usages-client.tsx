...
function EvidenceThumb({ path, alt }: { path?: string | null; alt: string }) {
  const src = storageUrl("usage-evidence", path);
  if (!src) return <span className="thumb-empty"><ImageIcon size={16} /> Tidak ada</span>;
  return <a href={src} target="_blank" rel="noreferrer"><img className="table-thumb" src={src} alt={alt} /></a>;
}
...
export function UsagesClient({ readOnly = false }: { readOnly?: boolean }) {
  ...
  async function loadData() {
    ...
    setError(null);
    try {
      const requests = readOnly ? [fetch("/api/usages")] : [fetch("/api/technician-bag"), fetch("/api/usages")];
  ...
  const availableBagItems = useMemo(() => bagItems.map((bag) => {
    const alreadySelected = items.find((item) => item.bag_id === bag.id)?.qty ?? 0;
    return { ...bag, remaining_qty: bag.qty - alreadySelected };
  }).filter((bag) => bag.remaining_qty > 0), [bagItems, items]);

  function resetForm() {
    setItems([]); setNoTiket(""); setNamaPelanggan(""); setIdPelanggan(""); setAlamat(""); setRootCause(""); setFoto(null); setSelectedBagId(""); setQty(1);
