...
export function MaterialsClient({ initialMaterials, initialMeta }: { initialMaterials: Material[]; initialMeta: Meta }) {
  ...
  async function refresh(page = meta.page) {
    setLoading(true);
    try {
      const res = await fetch(`/api/materials?page=${page}&limit=${meta.limit}`, { cache: "no-store" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Gagal memuat data material.");
      setMaterials(json.data || []);
      setMeta(json.meta || meta);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Gagal memuat data material.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function addSerialAndFocus(index?: number) {
    const nextIndex = index == null ? form.serial_numbers.length : index + 1;
    setForm((current) => ({ ...current, serial_numbers: [...current.serial_numbers, ""] }));
    setTimeout(() => serialRefs.current[nextIndex]?.focus(), 50);
  }
  ...
  async function submit() {
    ...
    try {
      const payload = { material_code: normalizeCode(form.material_code), nama: form.nama.trim(), merk: form.merk.trim(), satuan: normalizeCode(form.satuan), kondisi_default: form.kondisi_default.trim(), min_stock: Number(form.min_stock), wajib_sn: form.wajib_sn, qty_awal: form.wajib_sn ? 0 : Number(form.qty_awal), serial_numbers: form.wajib_sn ? form.serial_numbers.map(normalizeCode).filter(Boolean) : [] };
      ...
      toast.success("Material berhasil disimpan."); closeModal(); await refresh(1);
  ...
  return <>
    <section className="card">
      ...
      <div className="table-wrap"><table><thead><tr><th>Material ID</th><th>Nama Material</th><th>Merk</th><th>Satuan</th><th>Kondisi</th><th>Min Stok</th><th>Wajib SN</th><th>Stok Gudang</th></tr></thead><tbody>{filtered.map((m) => <tr key={m.id}><td><strong>{m.material_code}</strong></td><td>{m.nama}</td><td>{m.merk}</td><td>{m.satuan}</td><td><span className="badge badge-muted">{m.kondisi_default}</span></td><td>{m.min_stock}</td><td><span className={m.wajib_sn ? "badge badge-warning" : "badge badge-muted"}>{m.wajib_sn ? "YA" : "TIDAK"}</span></td><td><strong>{m.gudang_qty ?? 0}</strong></td></tr>)}{filtered.length === 0 && <tr><td colSpan={8}><div className="empty-state">Belum ada data material.</div></td></tr>}</tbody></table></div>
