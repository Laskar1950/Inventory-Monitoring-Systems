...
type EditState = { material_code: string; nama: string; merk: string; satuan: string; kondisi_default: string; min_stock: string };
type Step = 1 | 2;
...
function makeEditState(m: Material): EditState { return { material_code: m.material_code, nama: m.nama, merk: m.merk, satuan: m.satuan, kondisi_default: normalizeCondition(m.kondisi_default), min_stock: String(m.min_stock ?? 0) }; }
...
export function MaterialsClient({ initialMaterials, initialMeta }: { initialMaterials: Material[]; initialMeta: Meta }) {
  ...
  async function refresh(page = meta.page) {
    setLoading(true);
    try { const res = await fetch(`/api/materials?page=${page}&limit=${meta.limit}`, { cache: "no-store" }); const json = await res.json(); if (!res.ok) throw new Error(json.error || "Gagal memuat data material."); setMaterials(json.data || []); setMeta(json.meta || meta); }
  ...
  function openDetail(material: Material) { setDetail(material); setEdit(makeEditState(material)); }

  function validateBase() {
    const min = Number(form.min_stock); const qty = Number(form.qty_awal);
    ...
    if (!form.kondisi_default.trim()) return "Kondisi material wajib diisi.";
  ...
  async function saveEdit() {
    ...
    setLoading(true);
    try { const res = await fetch("/api/materials", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: detail.id, material_code: normalizeCode(edit.material_code), nama: edit.nama.trim(), merk: edit.merk.trim(), satuan: normalizeCode(edit.satuan), kondisi_default: normalizeCondition(edit.kondisi_default), min_stock: min }) }); const json = await res.json(); if (!res.ok) throw new Error(json.error || "Gagal memperbarui material."); toast.success(json.message || "Material berhasil diperbarui."); setDetail(null); setEdit(null); await refresh(meta.page); }
  ...
  async function deleteMaterial() {
    if (!detail) return;
    if (!confirm(`Hapus material ${detail.material_code} - ${detail.nama}? Data akan disembunyikan dari daftar aktif.`)) return;
    setLoading(true);
    try { const res = await fetch(`/api/materials?id=${detail.id}`, { method: "DELETE" }); const json = await res.json(); if (!res.ok) throw new Error(json.error || "Gagal menghapus material."); toast.success(json.message || "Material berhasil dihapus."); setDetail(null); setEdit(null); await refresh(meta.page); }
    catch (error) { toast.error(error instanceof Error ? error.message : "Gagal menghapus material."); }
    finally { setLoading(false); }
  }
