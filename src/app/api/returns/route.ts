...
export async function GET() {
  ...
  let query = supabase.from("material_return_summary").select("*").order("created_at", { ascending: false });
...
export async function POST(request: Request) {
  ...
  if (profile.role !== "TEKNISI") {
    return NextResponse.json({ error: "Hanya Teknisi yang dapat mengirim pengembalian material." }, { status: 403 });
  }
  ...
  try {
    ...
    try {
      items = JSON.parse(itemsRaw) as ReturnItemPayload[];
    } catch {
      return NextResponse.json({ error: "Payload item return tidak valid." }, { status: 400 });
    }

    if (!Array.isArray(items) || items.length === 0) {
      return NextResponse.json({ error: "Minimal satu material pengembalian harus ditambahkan." }, { status: 400 });
    }
    ...
    for (const item of items) {
      ...
      if (sourceType === "MANUAL") {
        if (!item.material_code || !item.nama || !item.merk || !item.satuan) {
          return NextResponse.json({ error: "Material ID, nama, merk, dan satuan wajib diisi untuk return manual." }, { status: 400 });
        }
        if (item.wajib_sn && (!item.serial_number || qty !== 1)) {
          return NextResponse.json({ error: "Material manual berserial wajib memiliki SN dan qty harus 1." }, { status: 400 });
        }
      }
    }
    ...
    const { error: uploadError } = await supabase.storage
      ...
      .upload(filePath, buffer, { contentType: foto.type, upsert: false });
    ...
    const normalizedItems = items.map((item) => ({
      ...
      material_code: item.material_code ? String(item.material_code).toUpperCase().trim() : null,
      nama: item.nama ? String(item.nama).trim() : null,
      merk: item.merk ? String(item.merk).trim() : null,
      satuan: item.satuan ? String(item.satuan).toUpperCase().trim() : null,
      wajib_sn: Boolean(item.wajib_sn),
      serial_number: item.serial_number ? String(item.serial_number).toUpperCase().trim() : null,
      qty: Number(item.qty),
    }));

    const { data, error } = await supabase.rpc("create_material_return", {
    ...
    if (error) {
      await supabase.storage.from("return-evidence").remove([filePath]);
      return NextResponse.json({ error: error.message || "Gagal menyimpan pengembalian material." }, { status: 400 });
    }
