...
export async function POST(request: Request) {
  ...
  if (parsed.data.basecamp || parsed.data.referensi_pekerjaan) {
    await supabase.from("material_requests").update({ basecamp: parsed.data.basecamp ?? null, referensi_pekerjaan: parsed.data.referensi_pekerjaan ?? null }).eq("id", data);
  }

  await notifyByRole(["LEADER", "ADMIN"], {
