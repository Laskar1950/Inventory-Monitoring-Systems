...
const requestSchema = z.object({ catatan_teknisi: z.string().optional().nullable(), items: z.array(z.object({ material_id: z.string().uuid(), qty: z.number().int().positive() })).min(1) });

async function getRequestItems(requestIds: string[]) {
  if (requestIds.length === 0) return new Map<string, any[]>();
  const supabase = createAdminClient();
  const { data, error } = await supabase.from("material_request_items").select("id,request_id,material_id,qty_requested,qty_approved,status,materials(material_code,nama,merk,satuan,wajib_sn)").in("request_id", requestIds).order("created_at", { ascending: true });
  ...
  for (const row of data ?? []) {
    ...
    list.push(item); map.set(row.request_id, list);
  }
  return map;
}
