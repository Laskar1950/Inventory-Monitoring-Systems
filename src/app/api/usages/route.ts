...
export async function GET() {
  ...
  let query = supabase.from("material_usage_summary").select("*").order("created_at", { ascending: false });
...
export async function POST(request: Request) {
  ...
  if (profile.role !== "TEKNISI") {
    return NextResponse.json({ error: "Hanya Teknisi yang dapat mencatat penggunaan material." }, { status: 403 });
  }
  ...
  try {
    ...
    try {
      items = JSON.parse(itemsRaw) as UsageItemPayload[];
    } catch {
      return NextResponse.json({ error: "Payload item penggunaan tidak valid." }, { status: 400 });
    }

    if (!Array.isArray(items) || items.length === 0) {
      return NextResponse.json({ error: "Minimal satu material harus ditambahkan." }, { status: 400 });
    }

    for (const item of items) {
      if (!item.bag_id || !Number.isFinite(Number(item.qty)) || Number(item.qty) <= 0) {
        return NextResponse.json({ error: "Material dan qty penggunaan wajib valid." }, { status: 400 });
      }
    }
    ...
    if (uploadError) {
      return NextResponse.json({ error: `Foto gagal diupload: ${uploadError.message}` }, { status: 500 });
    }

    const { data, error } = await supabase.rpc("create_material_usage", {
      p_teknisi_id: profile.id,
      p_no_tiket: noTiket,
      p_nama_pelanggan: namaPelanggan || null,
      p_id_pelanggan: idPelanggan || null,
      p_alamat: alamat || null,
      p_root_cause: rootCause,
      p_foto_url: filePath,
      p_items: items.map((item) => ({ bag_id: item.bag_id, qty: Number(item.qty) })),
    });

    if (error) {
      await supabase.storage.from("usage-evidence").remove([filePath]);
      return NextResponse.json({ error: error.message || "Gagal menyimpan penggunaan material." }, { status: 400 });
    }
