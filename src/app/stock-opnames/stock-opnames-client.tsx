...
export function StockOpnamesClient() {
  ...
  async function loadData() {
    ...
    try {
      const [bagRes, soRes] = await Promise.all([fetch("/api/technician-bag", { cache: "no-store" }), fetch("/api/stock-opnames", { cache: "no-store" })]);
  ...
  async function submitStockOpname(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(json.message || "Laporan stok opname berhasil dikirim."); setCatatanTeknisi(""); setFormOpen(false); await loadData();
