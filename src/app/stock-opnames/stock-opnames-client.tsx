...
export function StockOpnamesClient() {
  ...
  async function loadData() {
    ...
    try {
      const [bagRes, soRes] = await Promise.all([fetch("/api/technician-bag"), fetch("/api/stock-opnames")]);
  ...
  async function submitStockOpname(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (draftItems.length === 0) { setError("Minimal harus ada satu item stok opname."); return; }
    for (const item of draftItems) {
      if (!Number.isFinite(Number(item.qty_physical)) || Number(item.qty_physical) < 0) { setError("Qty fisik wajib angka dan tidak boleh negatif."); return; }
      if (item.wajib_sn && ![0, 1].includes(Number(item.qty_physical))) { setError(`Qty fisik material berserial ${item.material_code} harus 0 atau 1.`); return; }
      if (!item.kondisi_fisik) { setError("Kondisi fisik wajib dipilih untuk setiap item."); return; }
      if (!item.foto) { setError(`Foto bukti wajib untuk material ${item.material_code}.`); return; }
    }
    setSubmitting(true);
    try {
      ...
      setMessage(json.message || "Laporan stok opname berhasil dikirim."); setCatatanTeknisi(""); await loadData();
  ...
  return <div className="page-grid">
    <section className="card">
      <div className="section-header"><div className="section-title"><h3>Kirim Stok Opname</h3><p>Periksa qty fisik, kondisi, dan upload foto bukti untuk setiap material di Tas Saya.</p></div><button className="btn-secondary" type="button" onClick={() => void loadData()} disabled={loading || submitting}>Refresh</button></div>
      ...
      <form onSubmit={submitStockOpname} className="form-stack">
        <div className="stat-grid three"><div className="stat-card mini"><span>Qty Sistem</span><strong>{totals.system}</strong></div><div className="stat-card mini"><span>Qty Fisik</span><strong>{totals.physical}</strong></div><div className="stat-card mini"><span>Selisih</span><strong>{totals.diff}</strong></div></div>
        <label><span>Catatan Umum</span><textarea value={catatanTeknisi} onChange={(e) => setCatatanTeknisi(e.target.value)} rows={2} placeholder="Catatan tambahan jika ada" /></label>
        <div className="table-wrap"><table className="table compact"><thead><tr><th>Material</th><th>Serial Number</th><th>Qty Sistem</th><th>Qty Fisik</th><th>Kondisi Fisik</th><th>Foto Bukti</th><th>Selisih</th></tr></thead><tbody>{loading ? <tr><td colSpan={7}>Memuat data...</td></tr> : draftItems.length === 0 ? <tr><td colSpan={7}>Tas teknisi kosong. Tidak ada item untuk stok opname.</td></tr> : draftItems.map((item, index) => <tr key={item.bag_id}><td><strong>{item.material_code}</strong><br /><span className="muted-text">{item.material_nama}</span></td><td>{item.serial_number || "-"}</td><td>{item.qty_system}</td><td><input type="number" min={0} max={item.wajib_sn ? 1 : undefined} value={item.qty_physical} onChange={(e) => updateItem(item.bag_id, { qty_physical: Number(e.target.value) })} className="table-input" /></td><td><select value={item.kondisi_fisik} onChange={(e) => updateItem(item.bag_id, { kondisi_fisik: e.target.value })} className="table-input"><option value="BAIK">Baik</option><option value="RUSAK">Rusak</option><option value="HILANG">Hilang</option><option value="PERLU CEK">Perlu Cek</option></select></td><td><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => updateItem(item.bag_id, { foto: e.target.files?.[0] ?? null })} aria-label={`Foto bukti ${index + 1}`} /></td><td>{Number(item.qty_physical) - item.qty_system}</td></tr>)}</tbody></table></div>
        <button className="btn-primary" type="submit" disabled={loading || submitting || draftItems.length === 0}>{submitting ? "Mengirim..." : "Kirim Laporan SO"}</button>
      </form>
    </section>
    <section className="card"><div className="section-title"><h3>Riwayat Stok Opname</h3><p>Status laporan akan berubah setelah Admin Gudang menyelesaikan review.</p></div><div className="table-wrap"><table className="table"><thead><tr><th>Kode</th><th>Tanggal</th><th>Status</th><th>Item</th><th>Qty Sistem</th><th>Qty Fisik</th><th>Selisih</th></tr></thead><tbody>{loading ? <tr><td colSpan={7}>Memuat data...</td></tr> : summaries.length === 0 ? <tr><td colSpan={7}>Belum ada stok opname.</td></tr> : summaries.map((row) => <tr key={row.id}><td><strong>{row.so_code}</strong></td><td>{formatDate(row.created_at)}</td><td><span className={`badge ${row.status === "APPROVED" ? "success" : row.status === "PENDING" ? "warning" : "danger"}`}>{statusLabel(row.status)}</span></td><td>{row.item_count}</td><td>{row.total_system_qty}</td><td>{row.total_physical_qty}</td><td>{row.total_selisih}</td></tr>)}</tbody></table></div></section>
  </div>;
}
