...
import { Plus, RefreshCcw, UploadCloud, X } from "lucide-react";
...
export function StockOpnamesClient() {
  ...
  async function submitRevision() {
    ...
    try {
      const formData = new FormData();
      formData.append("items", JSON.stringify(revisionItems.map((item, index) => ({ item_id: item.id, file_key: `foto_revisi_${index}` }))));
  ...
  return <div className="page-grid">
    <section className="card clean-card-header"><div className="section-header"><div className="section-title"><h3>Riwayat Stok Opname</h3><p>Status laporan akan berubah setelah Admin Gudang menyelesaikan review.</p></div><div className="action-row"><button className="btn-primary" type="button" onClick={() => setFormOpen(true)}><Plus size={16}/> Tambah Stok Opname</button><button className="btn-secondary" type="button" onClick={() => void loadData(meta.page)} disabled={loading || submitting}><RefreshCcw size={15}/> Refresh</button></div></div>{message && <div className="alert-success">{message}</div>}{error && <div className="alert-error">{error}</div>}<div className="table-wrap"><table className="table"><thead><tr><th>Kode</th><th>Tanggal</th><th>Status</th><th>Item</th><th>Qty Sistem</th><th>Qty Fisik</th><th>Selisih</th><th>Aksi</th></tr></thead><tbody>{loading ? <TableSkeleton rows={6} columns={8} /> : summaries.length === 0 ? <tr><td colSpan={8}>Belum ada stok opname.</td></tr> : summaries.map((row) => <tr key={row.id}><td><strong>{row.so_code}</strong></td><td>{formatDate(row.created_at)}</td><td><span className={`badge ${row.status === "APPROVED" ? "success" : row.status === "PENDING" ? "warning" : "danger"}`}>{statusLabel(row.status)}</span></td><td>{row.item_count}</td><td>{row.total_system_qty}</td><td>{row.total_physical_qty}</td><td>{row.total_selisih}</td><td>{row.status === "REVISION" ? <button className="btn-primary-small" type="button" onClick={() => { setRevisionOpenId(row.id); setRevisionFiles({}); }}><UploadCloud size={14}/> Update Revisi</button> : <span className="muted-text">-</span>}</td></tr>)}</tbody></table></div><PaginationBar meta={meta} loading={loading} onPageChange={(page) => void loadData(page)} /></section>
    ...
    {revisionOpenId && revisionSummary && <div className="modal-backdrop request-detail-backdrop"><div className="modal stock-opname-review-modal"><div className="modal-header compact"><div><h3 className="modal-title">Update Revisi Stok Opname</h3><div className="modal-subtitle">{revisionSummary.so_code} • Upload ulang foto untuk item yang diminta revisi.</div></div><button className="btn-ghost" type="button" onClick={() => setRevisionOpenId(null)} disabled={submitting}><X size={16}/></button></div><div className="modal-body stock-opname-review-body"><div className="table-wrap compact-detail-table"><table className="table compact"><thead><tr><th>Material</th><th>Serial Number</th><th>Foto Lama</th><th>Catatan Admin</th><th>Foto Revisi</th><th>Preview</th></tr></thead><tbody>{revisionItems.map((item, index) => <tr key={item.id}><td><strong>{item.material_nama}</strong><br/><span className="muted-text">{item.material_code}</span></td><td>{item.serial_number || "-"}</td><td><EvidencePhoto path={item.foto_url} alt={`Foto lama ${item.material_nama}`} /></td><td>{item.catatan_admin || "-"}</td><td><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => setRevisionFiles((prev) => ({ ...prev, [item.id]: e.target.files?.[0] ?? null }))} aria-label={`Foto revisi ${index + 1}`} /></td><td><FilePreview file={revisionFiles[item.id] || null} alt={`Preview revisi ${item.material_nama}`} /></td></tr>)}</tbody></table></div></div><div className="modal-footer"><button className="btn-primary" type="button" onClick={() => void submitRevision()} disabled={submitting}>{submitting ? "Mengirim revisi..." : "Kirim Revisi"}</button></div></div></div>}
  </div>;
}
