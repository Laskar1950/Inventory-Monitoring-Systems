...
export function ApprovalRequestsClient({ initialRequests, initialMeta }: { initialRequests: RequestDetail[]; initialMeta: Meta }) {
  ...
  return <section className="card clean-card-header approval-page-card">
    ...
    <ConfirmDialog open={!!pendingAction} title="Proses Surat Jalan?" message={`Request ${pendingAction?.row.request_code} akan diproses menjadi Surat Jalan dan masuk ke tahap tanda tangan Koordinator.`} confirmLabel="Ya, Proses" variant="primary" loading={!!loadingId} onCancel={() => setPendingAction(null)} onConfirm={() => void executeSubmit()} />
  </section>;
}
