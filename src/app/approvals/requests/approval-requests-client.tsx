...
type PendingAction = { row: RequestDetailWithSerials } | null;
...
function statusText(status: string) { return status === "LEADER_APPROVED" ? "Menunggu Koordinator" : status === "WAITING_SIGNATURE" ? "Menunggu Admin Gudang" : status === "KOORDINATOR_SIGNED" ? "Menunggu Supervisor" : status === "APPROVED" ? "Siap Diterima Teknisi" : status === "COMPLETED" ? "Selesai" : status; }
...
export function ApprovalRequestsClient({ initialRequests, initialMeta }: { initialRequests: RequestDetail[]; initialMeta: Meta }) {
  ...
  async function openApproval(row: RequestDetail) {
    ...
    try {
      ...
      setApproval(data);
      const draft = data.items.map((item) => ({ item_id: item.id, material_id: item.material_id, material_code: item.material_code, material_nama: item.material_nama, wajib_sn: item.wajib_sn, qty_requested: item.qty_requested, qty_approved: item.qty_approved ?? item.qty_requested, serial_ids: item.selected_serials?.map((s) => s.serial_number_id) ?? [] }));
  ...
  function updateDraft(itemId: string, patch: Partial<ApprovalDraftItem>) { setApprovalItems((current) => current.map((item) => item.item_id === itemId ? { ...item, ...patch } : item)); }
  ...
  async function executeSubmit() {
    ...
    setLoadingId(row.id);
    try {
      const payload = { catatan_admin: notes[row.id] ?? row.catatan_admin ?? "", items: approvalItems.map((item) => ({ item_id: item.item_id, qty_approved: item.qty_approved, serial_ids: item.serial_ids })) };
      const res = await fetch(`/api/requests/${row.id}/approve`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Approval gagal diproses.");
      toast.success(json.message || "Surat jalan berhasil dibuat.");
  ...
  return <section className="card clean-card-header approval-page-card">
    <div className="section-header"><div className="section-title"><h3>Setujui Permintaan Material</h3><p>Admin memproses request yang sudah disetujui Supervisor menjadi Surat Jalan/BAST.</p></div></div>
