...
function statusClass(status: string) { if (["APPROVED", "KOORDINATOR_SIGNED"].includes(status)) return "badge badge-success"; if (status === "REJECTED") return "badge badge-danger"; return "badge badge-warning"; }
function statusText(status: string) { return status === "LEADER_APPROVED" ? "Leader Approved" : status === "WAITING_SIGNATURE" ? "Menunggu Koordinator" : status === "KOORDINATOR_SIGNED" ? "Menunggu Supervisor" : status; }
