...
function statusClass(status: string) { if (["APPROVED", "KOORDINATOR_SIGNED", "WAITING_SIGNATURE"].includes(status)) return "badge badge-success"; if (status === "REJECTED") return "badge badge-danger"; return "badge badge-warning"; }
...
export function ApprovalRequestsClient({ initialRequests, initialMeta }: { initialRequests: RequestDetail[]; initialMeta: Meta }) {
  ...
  async function submitApproval(action: "approve" | "reject") {
    ...
    const confirmText = action === "approve" ? "Proses request ini menjadi Surat Jalan?" : "Tolak request ini?";
    if (!window.confirm(confirmText)) return;
    setLoadingId(approval.id);
    try {
      const payload = action === "approve"
        ? { catatan_admin: notes[approval.id] ?? approval.catatan_admin ?? "", items: approvalItems.map((item) => ({ item_id: item.item_id, qty_approved: item.qty_approved, serial_ids: item.serial_ids })) }
        : { catatan_admin: notes[approval.id] ?? approval.catatan_admin ?? "" };
      const res = await fetch(`/api/requests/${approval.id}/${action}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const json = await res.json();
      ...
      setApproval(null); setApprovalItems([]); setAvailableSerials({});
