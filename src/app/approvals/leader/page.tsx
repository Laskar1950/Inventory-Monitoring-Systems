...
export default async function LeaderApprovalPage() {
  const profile = await requireProfile(["LEADER"]);
  const initial = await getInitialData();
  return <AppShell profile={profile} title="Approval Request Leader"><WorkflowApprovalClient mode="LEADER" initialRequests={initial.data as any} initialMeta={{ page: 1, limit: LIMIT, total: initial.total, totalPages: Math.max(1, Math.ceil(initial.total / LIMIT)) }} /></AppShell>;
}
