...
export default async function KoordinatorApprovalPage() {
  const profile = await requireProfile(["KOORDINATOR"]);
  const initial = await getInitialData();
  return <AppShell profile={profile} title="Tanda Tangan Surat Jalan"><WorkflowApprovalClient mode="KOORDINATOR" initialRequests={initial.data as any} initialMeta={{ page: 1, limit: LIMIT, total: initial.total, totalPages: Math.max(1, Math.ceil(initial.total / LIMIT)) }} /></AppShell>;
}
