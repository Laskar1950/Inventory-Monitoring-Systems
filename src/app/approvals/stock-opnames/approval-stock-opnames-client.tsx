...
function statusClass(status: string) { return status === "APPROVED" ? "badge badge-success" : status === "PENDING" ? "badge badge-warning" : "badge badge-danger"; }
function uniqueMaterials(items: StockOpnameItem[]) { const names = Array.from(new Set(items.map((i) => i.material_nama).filter(Boolean))); return names; }

function EvidencePhoto({ path, alt }: { path: string; alt: string }) {
  const [src, setSrc] = useState("");
  useEffect(() => { let active = true; if (!path) { setSrc(""); return; } if (path.startsWith("http")) { setSrc(path); return; } fetch(`/api/storage-url?bucket=stock-opname-evidence&path=${encodeURIComponent(path)}`).then((r) => r.json()).then((j) => { if (active) setSrc(j.signedUrl || ""); }).catch(() => { if (active) setSrc(""); }); return () => { active = false; }; }, [path]);
...
export function ApprovalStockOpnamesClient() {
  ...
  function openReview(summary: StockOpnameSummary) {
    ...
    for (const item of items.filter((item) => item.stock_opname_id === summary.id)) {
      initial[item.id] = { status_review: item.status_review === "APPROVED" || item.status_review === "REVISION" || item.status_review === "REJECTED_FINAL" ? item.status_review : "APPROVED", catatan_admin: item.catatan_admin || "" };
    }
    setSelectedId(summary.id); setReviews(initial); setError(null);
  }
  ...
  async function submitReview() {
    ...
    try {
      ...
      toast.success(json.message || "Review stok opname berhasil disimpan."); setSelectedId(null); await loadData();
    } catch (err) { setError(err instanceof Error ? err.message : "Gagal menyimpan review stok opname."); }
    finally { setSubmitting(false); }
  }

  return <section className="card clean-card-header approval-so-card">
    ...
    <div className="table-toolbar"><button className="btn-secondary" type="button" onClick={() => void loadData()} disabled={loading || submitting}><RefreshCcw size={15}/> Refresh</button></div>
    <div className="table-wrap"><table className="table"><thead><tr><th>Kode SO</th><th>Tanggal</th><th>Teknisi</th><th>Status</th><th>Item</th><th>Material</th><th>Selisih</th><th>Problem</th><th>Aksi</th></tr></thead><tbody>{loading ? <tr><td colSpan={9}>Memuat data...</td></tr> : summaries.length === 0 ? <tr><td colSpan={9}>Belum ada stok opname.</td></tr> : summaries.map((row) => { const rowItems = items.filter((item) => item.stock_opname_id === row.id); const materials = uniqueMaterials(rowItems); return <tr key={row.id}><td><strong>{row.so_code}</strong></td><td>{formatDate(row.created_at)}</td><td>{row.teknisi_nama}</td><td><span className={statusClass(row.status)}>{statusLabel(row.status)}</span></td><td>{row.item_count}</td><td><div className="material-stack">{materials.length ? materials.map((name) => <span key={name}>{name}</span>) : <span>-</span>}</div></td><td>{row.total_selisih}</td><td>{row.problem_count}</td><td><button className="btn-primary-small" type="button" onClick={() => openReview(row)}><Eye size={14}/> {row.status === "PENDING" ? "Review" : "Detail"}</button></td></tr>; })}</tbody></table></div>
    {selectedSummary && <div className="modal-backdrop request-detail-backdrop"><div className="modal stock-opname-review-modal"><div className="modal-header compact"><div><h3 className="modal-title">{selectedSummary.status === "PENDING" ? "Review Stok Opname" : "Detail Stok Opname"}</h3><div className="modal-subtitle">{selectedSummary.so_code} • {selectedSummary.teknisi_nama} • {formatDate(selectedSummary.created_at)}</div></div><button className="btn-ghost" type="button" onClick={() => setSelectedId(null)} disabled={submitting}><X size={16}/></button></div><div className="modal-body stock-opname-review-body">{error && <div className="alert-error">{error}</div>}<div className="approval-note-box"><strong>Catatan Teknisi</strong><p>{selectedSummary.catatan_teknisi || "-"}</p></div><div className="table-wrap compact-detail-table"><table className="table compact"><thead><tr><th>Material</th><th>Serial Number</th><th>Qty Sistem</th><th>Qty Fisik</th><th>Selisih</th><th>Kondisi</th><th>Foto</th><th>Status Review</th><th>Catatan Admin</th></tr></thead><tbody>{selectedItems.map((item) => <tr key={item.id}><td><strong>{item.material_nama}</strong><br/><span className="muted-text">{item.material_code}</span></td><td>{item.serial_number || "-"}</td><td>{item.qty_system}</td><td>{item.qty_physical}</td><td>{item.selisih}</td><td>{item.kondisi_fisik}</td><td><EvidencePhoto path={item.foto_url} alt={`Foto ${item.material_nama}`} /></td><td><select className="table-input" value={reviews[item.id]?.status_review || "APPROVED"} onChange={(e) => updateReview(item.id, { status_review: e.target.value as ReviewState[string]["status_review"] })} disabled={selectedSummary.status !== "PENDING" || submitting}><option value="APPROVED">Approved</option><option value="REVISION">Revisi</option><option value="REJECTED_FINAL">Rejected Final</option></select></td><td><input className="table-input" value={reviews[item.id]?.catatan_admin || ""} onChange={(e) => updateReview(item.id, { catatan_admin: e.target.value })} placeholder="Wajib untuk Revisi/Rejected" disabled={selectedSummary.status !== "PENDING" || submitting} /></td></tr>)}</tbody></table></div></div><div className="modal-footer">{selectedSummary.status === "PENDING" && <button className="btn-primary" type="button" onClick={() => void submitReview()} disabled={submitting}><Save size={16}/>{submitting ? "Menyimpan..." : "Simpan Review"}</button>}</div></div></div>}
  </section>;
}
