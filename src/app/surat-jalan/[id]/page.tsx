...
function SignatureBlock({ title, role, name, phone, company, signatureUrl, signedSrc, className }: SignatureBlockProps) {
  return <div className={["sj-sign-box", className].filter(Boolean).join(" ")}>
    <strong>{title}</strong>
    <span>{role}</span>
  ...
  return <main className="sj-page-wrap">
    ...
    <section className="sj-sheet">
      <header className="sj-header">
        ...
        <img className="sj-logo-img" src="/pln-icon-plus-logo.svg" alt="PLN Icon Plus" />
      </header>
      ...
      <div className="sj-bottom-grid sj-signature-grid">
        <SignatureBlock title="Yang Menyerahkan" role="Tim Gudang" name={request.approved_by_nama} phone={request.approved_by_phone_number} company={request.approved_by_company_name || "PLN ICONPLUS"} signatureUrl={request.admin_signature_url} signedSrc={signatures.admin} />
        <div className="sj-checklist sj-receive-box">
          <div><div className="sj-receive-line"><strong>Material Telah Disiapkan</strong><span>:</span><span>{formatDateTime(request.admin_signed_at || request.approved_at)}</span><input type="checkbox" checked readOnly /></div><div className="sj-receive-line"><strong>Waktu Pengambilan Material</strong><span>:</span><span></span><input type="checkbox" readOnly /></div><div className="sj-receive-line"><strong>Durasi Transaksi</strong><span>:</span><span></span><input type="checkbox" readOnly /></div><div className="sj-receive-line"><strong>Catatan</strong><span>:</span><span>{request.catatan_admin || ""}</span><input type="checkbox" readOnly /></div></div>
          <div className="sj-technician-inline-sign"><div className="sj-receive-line"><strong>Penerima</strong><span>:</span><span></span><input type="checkbox" readOnly /></div><div className="sj-sign-space">{signatures.teknisi ? <img className="sj-sign-img" src={signatures.teknisi} alt="Tanda tangan teknisi" /> : <em>{request.teknisi_nama ? signatureText(request.teknisi_signature_url) : "Menunggu tanda tangan"}</em>}</div><b>{personLine(request.teknisi_nama, request.teknisi_phone_number)}</b><small>{request.teknisi_company_name || "-"}</small></div>
        </div>
        <SignatureBlock title="Yang Bertanggung Jawab atas Permintaan Material" role="Tim Management" name={request.koordinator_nama} phone={request.koordinator_phone_number} company={request.koordinator_company_name || "PLN ICONPLUS"} signatureUrl={request.koordinator_signature_url} signedSrc={signatures.koordinator} />
        <SignatureBlock title="Mengetahui & Menyetujui" role="Team Leader Pemeliharaan Ritel Jateng" name={request.supervisor_nama} phone={request.supervisor_phone_number} company={request.supervisor_company_name || "PT. PLN ICONPLUS"} signatureUrl={request.supervisor_signature_url} signedSrc={signatures.supervisor} />
      </div>
    </section>
  </main>;
}
