...
export default async function SuratJalanPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="sj-page-wrap">
    ...
    <section className="sj-sheet">
      ...
      <div className="sj-checklist"><label><strong>Material Telah Disiapkan</strong><span>: {formatDateTime(request.admin_signed_at || request.approved_at)}</span><input type="checkbox" checked readOnly /></label><label><strong>Waktu Pengambilan Material</strong><span>:</span><input type="checkbox" readOnly /></label><label><strong>Durasi Transaksi</strong><span>:</span><input type="checkbox" readOnly /></label><label><strong>Catatan</strong><span>: {request.catatan_admin || ""}</span><input type="checkbox" readOnly /></label><label><strong>Penerima</strong><span>: {request.teknisi_nama}</span><input type="checkbox" readOnly /></label></div>
      <div className="sj-bottom-grid sj-signature-grid"><SignatureBlock title="Yang Menyerahkan" role="Admin Gudang" name={request.approved_by_nama} phone={request.approved_by_phone_number} company={request.approved_by_company_name || "PLN ICONPLUS"} signatureUrl={request.admin_signature_url} signedSrc={signatures.admin} /><SignatureBlock title="Koordinator Mitra" role="Koordinator" name={request.koordinator_nama} phone={request.koordinator_phone_number} company={request.koordinator_company_name || "PLN ICONPLUS"} signatureUrl={request.koordinator_signature_url} signedSrc={signatures.koordinator} /><SignatureBlock title="Mengetahui & Menyetujui" role="Supervisor" name={request.supervisor_nama} phone={request.supervisor_phone_number} company={request.supervisor_company_name || "PT. PLN ICONPLUS"} signatureUrl={request.supervisor_signature_url} signedSrc={signatures.supervisor} /><SignatureBlock title="Yang Menerima" role="Teknisi" name={request.teknisi_nama} phone={request.teknisi_phone_number} company={request.teknisi_company_name || "-"} signatureUrl={request.teknisi_signature_url} signedSrc={signatures.teknisi} className="sj-technician-sign" /><SignatureBlock title="BAST Final" role="Penerimaan Material" name={request.received_at ? "Selesai" : "Menunggu"} company={request.received_at ? formatDateTime(request.received_at) : "Menunggu penerimaan teknisi"} /></div>
    </section>
  </main>;
}
