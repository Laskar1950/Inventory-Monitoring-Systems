...
export function RequestsClient({ initialMaterials, initialRequests, initialMeta }: { initialMaterials: Material[]; initialRequests: RequestDetail[]; initialMeta?: Meta }) {
  ...
  async function downloadPdf(row: RequestDetail) {
    setDownloadingId(row.id);
    try {
      const res = await fetch(`/api/requests/${row.id}/surat-jalan-pdf`, { cache: "no-store" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Gagal mengambil PDF Surat Jalan.");
      window.open(json.signedUrl, "_blank", "noopener,noreferrer");
    } catch (error) { toast.error(error instanceof Error ? error.message : "Gagal mengambil PDF Surat Jalan."); }
