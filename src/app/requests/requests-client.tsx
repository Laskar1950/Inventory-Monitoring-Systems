...
export function RequestsClient({ initialMaterials, initialRequests, initialMeta }: { initialMaterials: Material[]; initialRequests: RequestDetail[]; initialMeta?: Meta }) {
  ...
  const filteredRequests = useMemo(() => {
    ...
    return requests.filter((r) => [r.request_code, r.status, r.catatan_teknisi ?? "", r.surat_jalan_number ?? "", ...r.items.map((i) => `${i.material_code} ${i.material_nama}`)].some((v) => v.toLowerCase().includes(q)));
  }, [query, requests]);
  ...
  async function submit() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/requests", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ catatan_teknisi: catatan, items: cart }) });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Gagal mengirim request material.");
      toast.success("Request berhasil dikirim.");
      setOpen(false); setCart([]); setCatatan(""); setSelectedMaterialId("");
      await refresh(1);
  ...
  return <>
    <section className="card">
      ...
      <div className="table-wrap"><table><thead><tr><th>Kode Request</th><th>Surat Jalan</th><th>Tanggal</th><th>Item</th><th>Total Qty</th><th>Status</th><th>Catatan Admin</th><th>Aksi</th></tr></thead><tbody>{loading ? <TableSkeleton rows={6} columns={8} /> : filteredRequests.map((r) => <tr key={r.id}><td><strong>{r.request_code}</strong></td><td>{r.surat_jalan_number || "-"}</td><td>{formatDate(r.created_at)}</td><td>{r.items.map((i) => `${i.material_nama} (${i.qty_requested})`).join(", ")}</td><td><strong>{r.total_qty}</strong></td><td><span className={statusClass(r.status)}>{statusText(r.status)}</span></td><td>{r.catatan_admin ?? "-"}</td><td><div className="action-row">{r.surat_jalan_number && <a className="btn-secondary-small" href={`/surat-jalan/${r.id}`} target="_blank"><FileText size={14}/> Lihat SJ</a>}{r.status === "APPROVED" && <button className="btn-primary-small" type="button" onClick={() => setReceiveTarget(r)} disabled={receivingId === r.id}><CheckCircle2 size={14}/>{receivingId === r.id ? "Menerima..." : "Terima"}</button>}{r.status === "COMPLETED" && <a className="btn-primary-small" href={`/api/requests/${r.id}/surat-jalan-pdf/download`} target="_blank"><Download size={14}/> PDF</a>}</div></td></tr>)}{!loading && filteredRequests.length === 0 && <tr><td colSpan={8}><div className="empty-state">Belum ada request material.</div></td></tr>}</tbody></table></div>
