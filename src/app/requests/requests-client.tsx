...
import { CheckCircle2, FileText, Plus, RefreshCcw, Search, Trash2, X } from "lucide-react";
...
export function RequestsClient({ initialMaterials, initialRequests, initialMeta, requesterProfile }: { initialMaterials: Material[]; initialRequests: RequestDetail[]; initialMeta?: Meta; requesterProfile?: RequesterProfile }) {
  ...
  function resetForm() { setCart([]); setCatatan(""); setSelectedMaterialId(""); setReferensiPekerjaan(""); setBasecamp(requesterProfile?.basecamp || ""); }
  ...
  async function receiveMaterial(row: RequestDetail) {
    if (!window.confirm(`Terima material untuk ${row.request_code}? Tanda tangan profil Anda akan masuk ke BAST dan status request menjadi selesai.`)) return;
    setReceivingId(row.id);
    try {
      const res = await fetch(`/api/requests/${row.id}/receive`, { method: "POST" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || json.warning || "Gagal menerima material.");
