...
export function RequestsClient({ initialMaterials, initialRequests, initialMeta, requesterProfile }: { initialMaterials: Material[]; initialRequests: RequestDetail[]; initialMeta?: Meta; requesterProfile?: RequesterProfile }) {
  ...
  return <>
    <section className="card request-page-card">
      ...
      <div className="table-wrap"><table><thead><tr><th>Kode Request</th><th>Surat Jalan</th><th>Tanggal</th><th>Basecamp</th><th>Referensi</th><th>Item</th><th>Total Qty</th><th>Status</th><th>Catatan Admin</th><th>Aksi</th></tr></thead><tbody>{loading ? <TableSkeleton rows={6} columns={10} /> : filteredRequests.map((r) => <tr key={r.id}><td><strong>{r.request_code}</strong></td><td>{r.surat_jalan_number || "-"}</td><td>{formatDate(r.created_at)}</td><td>{r.basecamp || "-"}</td><td>{r.referensi_pekerjaan || "-"}</td><td>{itemNames(r.items)}</td><td><strong>{r.total_qty}</strong></td><td><span className={statusClass(r.status)}>{statusText(r.status)}</span></td><td>{r.catatan_admin ?? "-"}</td><td><div className="action-row"><a className="btn-detail-blue" href={`/surat-jalan/${r.id}`} target="_blank"><FileText size={14}/> Detail</a></div></td></tr>)}{!loading && filteredRequests.length === 0 && <tr><td colSpan={10}><div className="empty-state">Belum ada request material.</div></td></tr>}</tbody></table></div>
