...
export default async function Page() {
  ...
  const [{ data: materials }, { data: requests }] = await Promise.all([
    supabase.from("materials_with_stock").select("*").order("material_code"),
    supabase.from("material_request_summary").select("*").eq("teknisi_id", profile.id).order("created_at", { ascending: false }),
  ]);
  ...
  return (
    <AppShell profile={profile} title="Permintaan Material">
      <RequestsClient initialMaterials={(materials ?? []) as any} initialRequests={detailed as any} />
    </AppShell>
  );
}
