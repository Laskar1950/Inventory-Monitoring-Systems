...
export function ReturnsClient() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [bagRes, returnRes] = await Promise.all([fetch("/api/technician-bag", { cache: "no-store" }), fetch("/api/returns", { cache: "no-store" })]);
  ...
  const filteredReturns = useMemo(() => {
    const q = query.toLowerCase().trim(); if (!q) return returns;
    return returns.filter((row) => [row.status, row.source_type, row.kondisi, row.materials_returned ?? "", row.serial_numbers ?? "", row.catatan_admin ?? ""].some((value) => value.toLowerCase().includes(q)));
  }, [query, returns]);
  ...
  function addManualItem() {
    ...
    setItems((prev) => [...prev, { material_code: code, nama: manualNama.trim(), merk: manualMerk.trim(), satuan: manualSatuan.trim().toUpperCase(), wajib_sn: manualWajibSn, serial_number: manualSn.trim().toUpperCase() || undefined, qty: manualQty, label: `${code} - ${manualNama.trim()}${manualSn.trim() ? ` | SN: ${manualSn.trim().toUpperCase()}` : ""}` }]);
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formData.append("source_type", sourceType); formData.append("kondisi", kondisi); formData.append("keterangan", keterangan); formData.append("items", JSON.stringify(items.map(({ label, ...item }) => item))); formData.append("foto", foto);
      const res = await fetch("/api/returns", { method: "POST", body: formData }); const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Gagal mengirim pengembalian material.");
      toast.success(json.message || "Pengembalian berhasil dikirim."); resetForm(); setFormOpen(false); await loadData();
    } catch (error) { toast.error(error instanceof Error ? error.message : "Gagal mengirim pengembalian material."); }
    finally { setSubmitting(false); }
  }

  return <div className="page-grid">
    <section className="card"><div className="section-header"><div className="section-title"><h3>Riwayat Pengembalian Material</h3><p>Klik tambah pengembalian untuk return dari Tas Saya atau return manual.</p></div><div className="action-row"><button className="btn-primary" type="button" onClick={() => setFormOpen(true)}><Plus size={16}/> Tambah Pengembalian</button><button className="btn-ghost" onClick={() => void loadData()} disabled={loading || submitting}><RefreshCcw size={15}/> Refresh</button></div></div>
      ...
      <div className="table-wrap"><table><thead><tr><th>Tanggal</th><th>Sumber</th><th>Material</th><th>Serial Number</th><th>Qty</th><th>Kondisi</th><th>Foto</th><th>Status</th><th>Catatan Admin</th></tr></thead><tbody>{loading ? <tr><td colSpan={9}>Memuat data...</td></tr> : filteredReturns.map((row) => <tr key={row.id}><td>{formatDate(row.created_at)}</td><td>{row.source_type}</td><td>{row.materials_returned || "-"}</td><td>{row.serial_numbers || "-"}</td><td><strong>{row.qty_return}</strong></td><td>{row.kondisi}</td><td><ReturnThumb path={row.foto_url} /></td><td><span className={statusClass(row.status)}>{row.status}</span></td><td>{row.catatan_admin || "-"}</td></tr>)}{!loading && filteredReturns.length === 0 && <tr><td colSpan={9}><div className="empty-state">Belum ada pengembalian material.</div></td></tr>}</tbody></table></div>
    </section>
